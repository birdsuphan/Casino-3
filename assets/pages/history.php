...
$rc = mysqli_query($con,"SELECT COUNT(id) AS id FROM treasure_wins WHERE user_id = {$_SESSION['user_id']} ORDER BY id DESC");
$numrows = mysqli_fetch_array($rc);
...
$rc = mysqli_query($con,"SELECT COUNT(id) AS id FROM flip_wins WHERE user_id = {$_SESSION['user_id']} ORDER BY id DESC");
$numrows = mysqli_fetch_array($rc);
...
$query = mysqli_query($con,"SELECT id,datetime,bet_amt,win_amount,win,roll_number FROM flip_wins WHERE user_id = {$_SESSION['user_id']} ORDER BY id DESC LIMIT $offset, $records");
...
echo  '<td>' . $r['win_amount'] . '</td>';
echo  '<td>' . $victory . '</td>';
echo '<td><a href="ajax?method=show_details&id=' . $r['id'] . '" target="_blank">Details</a> / <a href="verify?method=flip&id=' . $r['id'] . '" target="_blank">Verify</a></td>';
echo '</tr>';
